...
def createDirectory(id, type, x=0):
    directory = os.path.join(
        '/home/iribarrenp/Geografia/static/uploaders/' + type + '/' + str(id))
...
def upload(id, type, request):
    ...
    app.config['UPLOAD_FOLDER'] = "/home/iribarrenp/Geografia/static/uploaders/" + \
        type + '/' + str(id)
...
def createPost(type):
    ...
    if request.method == 'POST' and comment_form.validate():
        ...
        if type == "Blog":
            ...
            post = TipeClass[type](title=title, subtitle=subtitle,
                                   body=body, category=category, user=user, image=image)
        else:
            post = TipeClass[type](
                title=title, subtitle=subtitle, body=body, user=user, image=image)
        db.session.add(post)
        db.session.commit()
        post = TipeClass[type].query.filter_by(
            title=title, subtitle=subtitle, body=body, user=user).first_or_404()
...
def show_post(idpost, type):
    ...
    post.pub_date = "Día " + str(post.pub_date)[8:10] + " del " + str(
        post.pub_date)[5:7] + " del año " + str(post.pub_date)[0:4]
    if type == 'Blog':
        category = Category.query.all()
    ...
    if request.method == 'POST' and login_form.validate():
        try:
            user = User.query.filter_by(
                email=login_form.email.data).first_or_404()
        except:
...
def getPublicList(type):
    nodo = Nodo.query.all()
    try:
        allObject = TipeClass[type].query.order_by(
            TipeClass[type].pub_date.desc()).all()
    except:
        return '<h1> No found type </h1>'
    if type == 'Blog':
        category = Category.query.all()
        return render_template('Frontal/list.html', posts=allObject, type='Blog', transType='Blog', category=category)
    return render_template('Frontal/list.html', nodo=nodo, posts=allObject, type=type, transType=translateNameSingle[type])
...
def getPublicListBlogCategory(idcategory):
    ...
    category = Category.query.all()
    nodo = Nodo.query.all()
    return render_template('Frontal/list.html', nodo=nodo, posts=allObject, type='Blog', transType='Blog', category=category)
...
def searchs():
    ...
    allObject = []
    for k in types:
        listType = TipeClass[k].query.order_by(
            TipeClass[k].pub_date.desc()).all()
        ...
        for i in listType:
            ...
            if exist:
                allObject.append(i)
    return render_template('Frontal/list.html', posts=allObject, nodo=nodo, type="Resultado de busqueda", transType="Resultado de busqueda")
...
# Mostrar mapa
@app.route('/Monitoring/exploradores')
def map_glaciar():
...
@app.route('/Monitoring/exploradores/<nameType>/<id>')
def prueba(nameType, id):
    df = pd.read_csv(
        "C:/Users/Sebastián-Durán/Documents/P R O Y E C T O S/Nueva carpeta/Geografia/static/csv_glaciar/example_baliza.csv")
    # df =
...
def show_casa():
    ...
    # Fecha ultimo dato
    data = pd.read_csv(
        "/home/iribarrenp/Geografia/static/monitoreo/casa/datos/bmp280.csv")
    data = data.iloc[:, 1:]
    # Renombro columnas porque se me dan vuelta :)
    data.columns = ["Date", "Temp", "Hum", "Press", "lluvia"]
    ...
    # Ruta ultima foto
    files = glob.glob(
        "/home/iribarrenp/Geografia/static/monitoreo/casa/fotos/"+"*.jpg")
